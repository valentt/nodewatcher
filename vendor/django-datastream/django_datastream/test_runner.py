...
class ResourceTestCase(test.ResourceTestCase):
    ...
    def assertMetaEqual(self, meta1, meta2):
        ...
        if meta1next is not None:
            meta1next = urlparse(meta1next)
            meta1next_query = parse_qs(meta1next.query, strict_parsing=True)
        if meta2next is not None:
            meta2next = urlparse(meta2next)
            meta2next_query = parse_qs(meta2next.query, strict_parsing=True)
        if meta1previous is not None:
            meta1previous = urlparse(meta1previous)
            meta1previous_query = parse_qs(meta1previous.query, strict_parsing=True)
        if meta2previous is not None:
            meta2previous = urlparse(meta2previous)
            meta2previous_query = parse_qs(meta2previous.query, strict_parsing=True)
